function [stTrain] = STInstantiate(stOldTrain, strTemporalType, tDuration, mCorrelation, fMemTau, fISIVar)

% STInstantiate - FUNCTION Convert spike train definitions into concrete trains
% $Id: STInstantiate.m 9695 2008-06-25 12:28:53Z dylan $
%
% Usage: [stTrain] = STInstantiate(stTrainDef, <'regular' / 'poisson' / 'gaussian' / 'gamma'>, ...
%                                  tDuration <, mCorrelation, fMemTau, fISIVar>)
%
% 'stTrainDef' is a spike train containing a simple train
% definition created by STCreate.  'tDuration' is the desired duration of the
% spike train in seconds.  The spike train generation algorithm is specified
% using one of {'regular', 'poisson', 'gaussian', 'gamma'}.  'mCorrelation'
% (described below) is used to generate sets of spike trains with
% precisely-specified cross correlations.  'fMemTau' (described below) is a
% memory time constant, in seconds, used to generate non-ergodic spike trains.
% 'fISIVar' defines the variance of inter-spike intervals used to generate the
% spike train.  This parameter is not supported by all generation algorithms.
%
% --- GENERATION ALGORITHMS
%
% Regular trains: These trains have precise inter-spike intervals, based on the
% instantaneous spiking frequency.  This degrades nicely for changing-frequency
% spike trains as well.  Non-ergodic and correlated trains are not supported by
% this algorithm.  Obviously.  'fISIVar' is ignored by this algorithm.
%
% Poisson trains: This algorithm generates spikes from homogeneous and
% inhomogeneous poisson processes (depending on whether the train definition has
% a constant frequency or not).  Non-ergodic and correlated spike trains can be
% generated by this algorithm.  'fISIVar' is ignored by this algorithm.
% Inhomogeneous spike trains are generated using the "thinning" algorithm of
% [1].
%
% Gaussian trains: This algorithm draws inter-spike intervals from a gaussian
% distribution, with mean frequency defined by a constant frequency spike train
% definition, and ISI variance defined by 'fISIVar'. This algorithm only
% supports constant frequency spike trains, and does not support correlated or
% non-ergodic trains.
%
% Gamma trains: This algorithm draws inter-spike intervals from a gamma
% distribution, with mean frequency defined by a constant frequency spike train
% definition, and ISI variance defined by 'fISIVar'. This algorithm only
% supports constant frequency spike trains, and does not support correlated or
% non-ergodic trains.
...
%
% To impose non-ergodicidy without correlations, provide an empty matrix for
% 'mCorrelation'.
%
% --- REFERENCES
%
...
bMemory = exist('fMemTau', 'var') && ~isempty(fMemTau);
...
if (exist('fISIVar', 'var'))
   nISIVarSize = numel(fISIVar);
   bArrayISIVar = nISIVarSize > 1;
else
   fISIVar = [];
   nISIVarSize = [];
   bArrayISIVar = false;
...
if (bArrayOutput)
   ...
   if (~bArrayISIVar && exist('fISIVar', 'var'))
      fISIVar = repmat(fISIVar, nNumTrains, 1);
      nVarISISize = numel(fISIVar);
   end
...
% - Use accelerated spike train generation, unless we want a correlated or
% non-ergodic spike train
bISIGenerationMode = ~bCorrelate & ~bMemory;

% - Pre-allocate spike test function array
fhTestSpike = cell(1, nNumTrains);

% - Check frequency profile and temporal type option combinations
for (nTrainIndex = 1:nNumTrains)
   switch lower(strTemporalType{nTrainIndex})
      % - Regular spike trains
      case {'regular', 'r'}
         % - Only the slow mode exists for regular spike trains
         fhTestSpike{nTrainIndex} = @STTestSpikeRegularSlow;

      % - Poisson spike trains
      case {'poisson', 'p'}
         fhTestSpike{nTrainIndex} = @STTestSpikePoisson;

      % - Normally distributed spike trains
      case {'normal', 'gaussian', 'n'}
         % - Were we given an ISI variance?
         if (~exist('fISIVar', 'var') || isempty(fISIVar(nTrainIndex)))
            disp('*** STInstantiate: You must supply an ISI variance to instantiate a spike train');
            disp('       with a temporal type of ''normal''.');
            return;
         end
         
         % - Check which frequency profiles we can generate
         switch lower(cDefinitions{nTrainIndex}.strType)
            case {'linear', 'sinusoid'}
               disp('*** STInstantiate: Spike trains created with a temporal type of');
               disp('       ''gamma'' can only be used for constant frequency trains.');
               return;
         end

         fhTestSpike{nTrainIndex} = @STTestSpikeGaussianISI;

      % - Gamma distributed spike trains
      case {'gamma', 'g'}
         % - Were we given an ISI variance?
         if (~exist('fISIVar', 'var') || isempty(fISIVar(nTrainIndex)))
            disp('*** STInstantiate: You must supply an ISI variance to instantiate a spike train');
            disp('       with a temporal type of ''gamma''.');
            return;
         end
         
         % - Check which spike train types we can generate
         switch lower(cDefinitions{nTrainIndex}.strType)
            case {'linear', 'sinusoid'}
               disp('*** STInstantiate: Spike trains created with a temporal type of');
               disp('       ''gamma'' can only be used for constant frequency trains.');
         ...
         fhTestSpike{nTrainIndex} = @STTestSpikeGammaISI;
         
         
      otherwise
         disp(['*** STInstantiate: The temporal spike train type [' strTemporalType{nTrainIndex} '] is not recognised']);
         help STInstantiate;
         return;
   end
end
...
% - Preallocate instance arrays
instance = cell(1, nNumTrains);
vbChunkedMode = false(1, nNumTrains);
vnNumChunks = zeros(1, nNumTrains);
...
cfhInstFreq = CellForEachCell(@subsref, cDefinitions, sRef);
...
for (nChunkIndex = 1:nNumChunks)
   ...
   sRef.type = '.';
   sRef.subs = 'fMaxFreq';
   vfMaxFreq = CellForEach(@subsref, cDefinitions, sRef);
   if (any((vfMaxFreq * InstanceTemporalResolution) > 1.0001))
   ...
   %   sequences
   if (bCorrelate)
      STProgress(' Generating correlated random sequence...');
   ...
   for (nTrainIndex = 1:nNumTrains)
      ...
      % - Get the last spike for this train
      if (nChunkIndex == 1)
         tLastSpike = [];
      else
         % - Check to make sure there WAS a spike in the last chunk
         if (isempty(instance{nTrainIndex}.spikeList{nChunkIndex-1}))
            tLastSpike = [];
         else
            tLastSpike = instance{nTrainIndex}.spikeList{nChunkIndex-1}(end);
         end
      end
      
      % - Get the ISI variance for this train
      if (~isempty(fISIVar))
         fISIVarCall = fISIVar(nTrainIndex);
      else
         fISIVarCall = [];
      end
      
      % - Generate the spike train
      spikeList = feval(fhTestSpike{nTrainIndex}, tTimeCurr, tLastSpike, ...
                           cfhInstFreq{nTrainIndex}, cDefinitions{nTrainIndex}, ...
                           vfCorrSeq, fMemTauItem, fISIVarCall);
...
for (nTrainIndex = 1:nNumTrains)
   instance = instance{nTrainIndex};
   
   % - Trim trains to desired duration (just to make sure...)
   if (vbChunkedMode(nTrainIndex))
      instance.spikeList{end} = ...
         instance.spikeList{end}(instance.spikeList{end} <= instance.tDuration);
   else
      instance.spikeList = instance.spikeList(instance.spikeList <= instance.tDuration);
   end
   
   % - Assign instance
   stTrain{nTrainIndex}.instance = instance;
end
